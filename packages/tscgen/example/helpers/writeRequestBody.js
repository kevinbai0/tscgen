...
const writeRequestBody = (requestBody, handlers) => {
    return writeSchema_1.handleRef(requestBody, {
        notRef: async (obj) => {
            const schemas = await tscgen.mapObjectPromise(obj?.content ?? {}, (mediaObj) => {
                return writeSchema_1.writeSchema(mediaObj.schema, handlers);
            });
